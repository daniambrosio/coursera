...
% -------------------------------------------------------------
% Part 1 - implementation (if lambda = 0; no regularization)
...
a2 = [ones(size(a2,1), 1) a2]; % Adding 1 as first column in z2 = (Adding bias unit) % m x (hidden_layer_size + 1) == 5000 x 26
z3 = a2 * Theta2'; % m x num_labels == 5000 x 10

a3 = sigmoid(z3); % m x num_labels == 5000 x 10
hTheta = a3; % m x num_labels == 5000 x 10
...
J = (1/m) * sum(sum((-y_Vec.*log(hTheta))-((1-y_Vec).*log(1-hTheta))));  

% -------------------------------------------------------------
% Part 2 - implementation 


% Backpropagation using a for loop
for t=1:m
